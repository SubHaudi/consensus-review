...
문서 검토에서 가장 흔한 실패는 "한 명의 LLM이 알아서 잘 리뷰해주길 기도하는 것"입니다. 6개 LLM × 3개 벤치마크(법률, 사실오류, 번역) × 10,200+ 정답 결함으로 검증한 결과, 같은 모델을 같은 프롬프트로 5번 돌리면 **매번 다른 결함**을 놓쳤습니다. 놓침은 "모르는 것"이 아니라 "확률적으로 놓치는 것"이었습니다.
